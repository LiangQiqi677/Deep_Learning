...
class Gru(torch.nn.Module):
    def __init__(self, input_size, hidden_size, output_size, embeding_vector):
        ...
        self.lstm = torch.nn.GRU(input_size=hidden_size, hidden_size=hidden_size, num_layers=1, batch_first=True)
    ...
    def forward(self, x, batch_seq_len,):
        ...
        return output[:, -1, :]  #torch.Size([2, 6]) 直接输出最后一列
...
model = Gru(input_size=input_size, hidden_size=hidden_size, output_size=6, embeding_vector=embedding_matrix).to(device)
model.load_state_dict(torch.load('./ex5_1.pt'))
...
for epoch in range(num_epoch):
    ...
    if test_loss[epoch] < loss_min:
        ...
        torch.save(model.state_dict(), './ex5_1.pt')
        print("save model...")
...
plt.ylabel("loss")
plt.xlabel("epoch")
plt.title("GRU Loss: batch_size=512, lr=0.01")
plt.legend()
plt.show()
...
plt.ylabel("accuracy ")
plt.xlabel("epoch")
plt.title("GRU Accuracy: batch_size=512, lr=0.01")
plt.legend()
plt.show()
...
plt.xlabel("epoch")
plt.title("GRU Precision: batch_size=512, lr=0.01")
plt.legend()
plt.show()
...
plt.ylabel("recall")
plt.xlabel("epoch")
plt.title("GRU Recall: batch_size=512, lr=0.01")
plt.legend()
plt.show()
...
plt.ylabel("f1")
plt.xlabel("epoch")
plt.title("GRU F1: batch_size=512, lr=0.01")
plt.legend()
plt.show()
